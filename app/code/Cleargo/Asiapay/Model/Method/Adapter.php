 ...
 */
namespace Cleargo\Asiapay\Model\Method;
...
{
    const STANDARD_FACADE = 'AsiapayStandardFacade';
     ...
     */
    public function __construct(ConfigInterface $config, ObjectManagerInterface $objectManager, \Psr\Log\LoggerInterface $logger)
    {
        $this->config = $config;
    ...
    private function getPaymentInstance()
    {
        if (!isset($this->paymentInstance)) {
            $facadeName = self::STANDARD_FACADE;
