...
class Edit extends \Magento\Backend\App\Action
{
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu('Cleargo_DealerNetwork::dealer_dealer')
            ->addBreadcrumb(__('Dealer Network'), __('Dealer Network'))
            ->addBreadcrumb(__('Manage Dealers'), __('Manage Dealers'));
        return $resultPage;
    }
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This dealer no longer exists.'));
        ...
        $this->_coreRegistry->register('dealer_dealer', $model);
        ...
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getName() : __('New Dealer'));
