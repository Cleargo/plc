...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This lock no longer exists.'));
        ...
        $this->_coreRegistry->register('lock_lock', $model);

        // 5. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Lock') : __('New Lock'),
            $id ? __('Edit Lock') : __('New Lock')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Locks'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getName() : __('New Lock'));
